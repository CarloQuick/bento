...
pub fn decompress_tarball(path: &str, destination: &str) -> Result<()> {
    ...
    archive.unpack(destination).context("Failed to unpack archive!")?;
...
pub fn unpack_archive(source: &PathBuf, dest: &PathBuf) -> Result<()> {
    ...
    let mut archive = Archive::new(File::open(source).with_context(|| format!("Failed to open archive at {:?}", source))?);
    archive.unpack(dest).with_context(|| format!("Failed to unpack archive to {:?}", dest))?;
